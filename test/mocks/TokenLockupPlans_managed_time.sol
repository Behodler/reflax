...
import '@hedgey/lockup/../ERC721Delegate/ERC721Delegate.sol';
import '@oz_hedgey/contracts/security/ReentrancyGuard.sol';
import '@hedgey/lockup/../libraries/TransferHelper.sol';
import '@hedgey/lockup/../libraries/TimelockLibrary.sol';
import '@hedgey/lockup/../sharedContracts/URIAdmin.sol';
import '@hedgey/lockup/../sharedContracts/LockupStorage.sol';
import {DebugTimeKeeper} from '../../src/governance/HedgeyAdapter.sol';
...
DebugTimeKeeper debugTimeKeeper;
  constructor(string memory name, string memory symbol, address vm_address) ERC721(name, symbol) {
    uriAdmin = msg.sender;
    debugTimeKeeper = new DebugTimeKeeper (Vm(vm_address));
  }
  ...
  /// @dev this function will redeem all claimable and unlocked tokens up to the current debugTimeKeeper.timestamp()
  ...
  function redeemPlans(uint256[] calldata planIds) external nonReentrant {
    _redeemPlans(planIds, debugTimeKeeper.timestamp());
  }
  ...
  function partialRedeemPlans(uint256[] calldata planIds, uint256 redemptionTime) external nonReentrant {
    require(redemptionTime < debugTimeKeeper.timestamp(), '!future');
  ...
  /// @dev this will iterate through all of the plans owned by the wallet based on the ERC721Enumerable backbone, and redeem each one with a redemption time of the current debugTimeKeeper.timestamp()
  ...
  /// @param redemptionTime is the requested redemption time, either the current debugTimeKeeper.timestamp() or a timestamp from the past, but must be greater than the start date
